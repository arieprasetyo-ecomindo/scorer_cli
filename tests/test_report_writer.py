...
STRUCTURE_SCORES = {
    "coupling": {"score_0_10": 7.7, "level": 3, "level_label": "Good"},
    "circular_dependencies": {"score_0_10": 10.0, "level": 4, "level_label": "Excellent"},
    "dependency_depth": {"score_0_10": 9.8, "level": 4, "level_label": "Excellent"},
    "cyclomatic_complexity": {"score_0_10": 6.5, "level": 3, "level_label": "Fair"},
    "function_size_discipline": {"score_0_10": 10.0, "level": 4, "level_label": "Excellent"},
    "betweenness_centrality": {"score_0_10": 9.5, "level": 4, "level_label": "Excellent"},
}
...
def test_write_markdown_report_includes_all_sections(tmp_path):
    path = write_markdown_report(
        "team-x", tmp_path, STRUCTURE_SCORES, SPEC_SCORES, "## Summary\n\nSolid.", ["a flag"], WEIGHTS
    )
    text = path.read_text()
...
def test_write_markdown_report_no_red_flags(tmp_path):
    path = write_markdown_report("team-x", tmp_path, STRUCTURE_SCORES, None, "## Summary\n\nOk.", [], WEIGHTS)
    text = path.read_text()
